...
type SubscriberRow = { id: number; email: string; subscribed_at: string; source: string };
...
function AdminDashboardInner({ user }: { user: { role: "admin" | "user"; isDefaultAdmin: number } }) {
  ...
  const subscribersQuery = trpc.cms.subscribers.useQuery(undefined, { enabled: isAdmin, retry: false });
  ...
  const createJournal = trpc.cms.createJournal.useMutation({ onSuccess: () => { toast.success("Journal post published."); content.refetch(); void utils.content.all.invalidate(); void utils.content.journalBySlug.invalidate(); resetJournal(); } });
  const updateJournal = trpc.cms.updateJournal.useMutation({ onSuccess: () => { toast.success("Journal post updated."); content.refetch(); void utils.content.all.invalidate(); void utils.content.journalBySlug.invalidate(); resetJournal(); } });
  const deleteJournal = trpc.cms.deleteJournal.useMutation({ onSuccess: () => { toast.success("Journal post removed."); content.refetch(); void utils.content.all.invalidate(); void utils.content.journalBySlug.invalidate(); } });
  ...
  const deleteSubscriber = trpc.cms.deleteSubscriber.useMutation({ onSuccess: () => { toast.success("Subscriber removed."); subscribersQuery.refetch(); } });

  const section = location === "/admin/events" ? "events" : location === "/admin/images" ? "images" : location === "/admin/canvases" ? "canvases" : location === "/admin/journal" ? "journal" : location === "/admin/enquiries" ? "enquiries" : location === "/admin/users" ? "users" : location === "/admin/orders" ? "orders" : location === "/admin/subscribers" ? "subscribers" : "overview";
  ...
  const subscriberRows = subscribersQuery.data as SubscriberRow[] | undefined;
  ...
  async function saveJournal() {
    try {
      const uploaded = await maybeUpload(imageFile);
      const dataToSave = { ...journalForm, slug: slugify(journalForm.slug || journalForm.title), imageUrl: uploaded?.url ?? journalForm.imageUrl, imageKey: uploaded?.key ?? (journalForm.imageKey || null), publishedAt: new Date(journalForm.publishedAt) };
  ...
  return <div className="cms-shell">
    ...
    {section === "subscribers" && <><DashboardHeader title="Newsletter subscribers" description="Keep track of people who asked to hear about new work, events, and studio notes." /><section className="cms-panel cms-subscriber-list">{subscriberRows?.length ? subscriberRows.map((subscriber) => <article className="cms-subscriber-record" key={subscriber.id}><div><span className="cms-record-status">{subscriber.source}</span><h2>{subscriber.email}</h2><small>Joined {new Date(subscriber.subscribed_at).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</small></div><button onClick={() => { if (window.confirm(`Remove ${subscriber.email} from the list?`)) deleteSubscriber.mutate({ id: subscriber.id }); }} aria-label={`Delete subscriber ${subscriber.email}`}><Trash2 size={15} /></button></article>) : <div className="cms-empty-state"><Inbox size={28} /><h2>No subscribers yet.</h2><p>New footer signups will appear here.</p></div>}</section></>}
