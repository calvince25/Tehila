...
import { ArrowUpRight, CalendarDays, Check, FileText, ImagePlus, Inbox, LayoutDashboard, LogOut, Pencil, Plus, Save, Trash2, Upload, UserCheck, Users, X } from "lucide-react";
...
function AdminDashboardInner({ user }: { user: { role: "admin" | "user"; isDefaultAdmin: number } }) {
  ...
  const section = location === "/admin/events" ? "events" : location === "/admin/images" ? "images" : location === "/admin/canvases" ? "canvases" : location === "/admin/journal" ? "journal" : location === "/admin/enquiries" ? "enquiries" : location === "/admin/users" ? "users" : "overview";
  ...
  return <div className="cms-shell">
    ...
    {section === "canvases" && <><DashboardHeader title="Shop canvases" description="Add, edit, or remove the original pieces shown in the shop." onAdd={resetCanvas} /><div className="cms-two-column"><section className="cms-panel cms-list">{data?.canvases?.map((item) => <div className="cms-record" key={item.id}><img src={resolveAdminImage(item.imageUrl)} alt={item.title} /><div><span className="cms-record-status">{item.status.replace("_", " ")}</span><h3>{item.title}</h3><p>{item.kind} · {item.size} · {item.price}</p></div><div className="cms-record-actions"><button onClick={() => editCanvas(item)} aria-label={`Edit ${item.title}`}><Pencil size={15} /></button><button onClick={() => deleteCanvas.mutate({ id: item.id })} aria-label={`Delete ${item.title}`}><Trash2 size={15} /></button></div></div>)}</section><section className="cms-panel cms-form-panel"><FormTitle title={editingCanvas ? "Edit canvas" : "New canvas"} onClose={editingCanvas ? resetCanvas : undefined} /><div className="cms-form-grid"><Field label="Title" value={canvasForm.title} onChange={(value) => setCanvasForm({ ...canvasForm, title: value })} /><Field label="Kind" value={canvasForm.kind} onChange={(value) => setCanvasForm({ ...canvasForm, kind: value })} /><Field label="Price (KSh)" value={canvasForm.price} onChange={(value) => setCanvasForm({ ...canvasForm, price: value })} /><Field label="Size" value={canvasForm.size} onChange={(value) => setCanvasForm({ ...canvasForm, size: value })} /><label className="cms-field"><span>Status</span><select value={canvasForm.status} onChange={(event) => setCanvasForm({ ...canvasForm, status: event.target.value as CanvasForm["status"] })}><option value="available">Available</option><option value="one_of_one">One of one</option><option value="coming_soon">Coming soon</option><option value="sold">Sold</option></select></label><Field label="Sort order" type="number" value={String(canvasForm.sortOrder)} onChange={(value) => setCanvasForm({ ...canvasForm, sortOrder: Number(value) })} /></div><TextField label="Description" value={canvasForm.description} onChange={(value) => setCanvasForm({ ...canvasForm, description: value })} /><ImagePicker file={canvasFile} currentUrl={canvasForm.imageUrl} onFile={setCanvasFile} /><button className="cms-primary cms-save" onClick={saveCanvas} disabled={upload.isPending || createCanvas.isPending || updateCanvas.isPending}><Save size={16} /> {editingCanvas ? "Save changes" : "Add canvas"}</button></section></div></>}
