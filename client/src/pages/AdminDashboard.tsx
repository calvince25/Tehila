...
import { ArrowUpRight, CalendarDays, FileText, ImagePlus, LayoutDashboard, LogOut, Pencil, Plus, Save, Trash2, Upload, X } from "lucide-react";
...
function AdminDashboardInner() {
  ...
  const section = location === "/admin/events" ? "events" : location === "/admin/images" ? "images" : location === "/admin/canvases" ? "canvases" : location === "/admin/journal" ? "journal" : "overview";
  const data = content.data as { canvases: CanvasRow[]; events: EventRow[]; images: ImageRow[]; journal: JournalRow[] } | undefined;
  const counts = useMemo(() => ({ canvases: data?.canvases?.length ?? 0, events: data?.events?.length ?? 0, images: data?.images?.length ?? 0, journal: data?.journal?.length ?? 0 }), [data]);
