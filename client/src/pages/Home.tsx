...
export default function Home() {
  const { data: content, isLoading, isFetching, isStale } = trpc.content.all.useQuery();
  ...
  return (
    <main className="studio-app">
      ...
      <header className="app-header">
        ...
        <nav className={`app-nav ${menuOpen ? "open" : ""}`}>{[["home", "Home"], ["story", "Story"], ["shop", "Shop"], ["events", "Events"], ["journal", "Journal"]].map(([id, label]) => <a key={id} href={`#${id}`} onClick={() => scrollTo(id)}>{label}</a>)}<a href="/contact">Contact</a></nav>
      ...
      <section className="app-hero" id="home">
        ...
        <div className="hero-collage"><div className="collage-ring ring-a" /><div className="collage-ring ring-b" /><figure className="portrait-card"><img src={media("maker")} alt={mediaAlt("maker", "Tehila making a string-art canvas")} fetchPriority="high" decoding="async" /><figcaption><span>01</span><span>At the studio table</span></figcaption></figure><figure className="art-card"><img src={media("pink")} alt={mediaAlt("pink", "Pink circular string-art canvas")} decoding="async" /><span>Orbit in Pink</span></figure><div className="collage-note">The work is<br /><em>the conversation.</em></div></div>
      </section>
